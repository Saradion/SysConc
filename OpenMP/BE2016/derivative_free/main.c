...
#pragma omp parallel
        ...
        xyz[current_thread][0] = xyz[0][0];
        xyz[current_thread][1] = xyz[0][1];
        xyz[current_thread][2] = xyz[0][2];
#pragma omp for
        for(i=0; i<p; i++){
            nx = x+ s*cos(2.0*M_PI*i/((double)p));
            ny = y+ s*sin(2.0*M_PI*i/((double)p));
            ...
            /* If the evaluation at this point is lower than the current
               minimum, set this point as the new minimum */
            if(nz<xyz[current_thread][2]){
                xyz[current_thread][2] = nz;
                xyz[current_thread][0] = nx;
                xyz[current_thread][1] = ny;
            }
        }
        /* Uncomment the line below if you want to debug */
        /* printf("%4d -- %5.2f %5.2f %10.4f\n",cnt,xyz[0][0], xyz[0][1], xyz[0][2]); */
        for (int count = 0; i < thread_count; i++) {
            if (xyz[count][2] < z) {
                xyz[0][0] = xyz[count][0];
                xyz[0][1] = xyz[count][1];
                xyz[0][2] = xyz[count][2];
